function [volt,m1,n1,h1] = rk4_q4(delta_t,T,input)
    ...
    , (0.01*(v+0.055))/(1-exp(-(v+0.055)/10))*(1-n)-n*0.125*exp(-(v+0.065)/80)==0 ...
    , (0.1*(v+0.040))/(1-exp(-(v+0.040)/10))*(1-m)-m*4*exp(-0.0556*(v+0.065))==0 ...
    , (0.07*exp(-0.005*(v+0.065)))*(1-h)-h*(1/1+(exp(-0.1*(v+0.035))))==0], ...
   [v,m,n,h] );
    ...
    G_xy = @(I,v,m,n,h) ((0.01*(v+0.055))/(1-exp(-(v+0.055)/0.010)))*(1-n)-n*(0.125*exp(-(v+0.065)/0.080))    ;
    H_xy = @(I,v,m,n,h) ((0.1*(v+0.040))/(1-exp(-(v+0.040)/0.010)))*(1-m)-m*(4*exp(-0.0556*(v+0.065)));
    I_xy = @(I,v,m,n,h) (0.07*exp(-0.05*(v+0.065)))*(1-h)-h*(1/(1+(exp(-0.1*(v+0.035)))));
